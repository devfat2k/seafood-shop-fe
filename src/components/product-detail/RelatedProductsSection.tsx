...
const RelatedProductItem = ({ rel, onAddToCart }: RelatedProductItemProps) => {
  const [imgError, setImgError] = useState(false);
  const relImage = rel.imageUrl ?? rel.images?.[0] ?? '';
  const relOriginalPrice = rel.originalPrice ?? Math.round(rel.price * 1.15);

  const isRelInStock = isProductInStock(rel);

  return (
    <div className="group flex flex-col justify-between overflow-hidden rounded-2xl border border-border bg-card shadow-xs transition-all duration-300 hover:-translate-y-1 hover:border-secondary/40 hover:shadow-md">
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-6 lg:grid-cols-4">
        {products.map((rel) => (
          <RelatedProductItem key={rel.id} rel={rel} onAddToCart={onAddToCart} />
        ))}
      </div>
    </section>
  );
