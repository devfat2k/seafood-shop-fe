...
export default defineConfig({
  ...
  overrides: [
    {
      // proxy.ts dùng Clerk's createRouteMatcher — API đang deprecated nhưng chưa có replacement stable
      files: ['src/proxy.ts'],
      rules: { 'typescript/no-deprecated': 'off' },
    },
    {
      // Shadcn UI primitives — boundary: không được sửa trực tiếp (AGENTS.md)
